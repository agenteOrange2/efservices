<div class="bg-white p-4 rounded-lg shadow">
    ...
    <div class="mb-6 border-b pb-4">
        <div class="mb-4">
            <label class="block text-sm font-medium text-gray-700 mb-1">Current License Number <span class="text-red-500">*</span></label>
            <input type="text" wire:model="current_license_number" class="w-full text-sm border-slate-200 shadow-sm rounded-md py-2 px-3">
        ...
        <div class="license-entry border rounded-lg p-4 mb-4">
            ...
            <div class="flex justify-between items-center mb-4">
                ...
                @if($index > 0)
                <button type="button" wire:click="removeLicense({{ $index }})" class="px-3 py-1 bg-red-500 text-white rounded hover:bg-red-600 text-sm">
                    Remove
                </button>
                @endif
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                <div>
                    ...
                    <input type="text" wire:model="licenses.{{ $index }}.license_number" placeholder="Enter license number" class="w-full text-sm border-slate-200 shadow-sm rounded-md py-2 px-3">
                    @error("licenses.{$index}.license_number")
                        <span class="text-red-500 text-sm">{{ $message }}</span>
                    @enderror
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">State of Issue <span class="text-red-500">*</span></label>
                    <select wire:model="licenses.{{ $index }}.state_of_issue" class="w-full text-sm border-slate-200 shadow-sm rounded-md py-2 px-3">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">License Class <span class="text-red-500">*</span></label>
                    <select wire:model="licenses.{{ $index }}.license_class" class="w-full text-sm border-slate-200 shadow-sm rounded-md py-2 px-3">
                ...
                <div>
                    ...
                    <input type="date" wire:model="licenses.{{ $index }}.expiration_date" class="w-full text-sm border-slate-200 shadow-sm rounded-md py-2 px-3">
            ...
            <div class="mb-4" x-data="{ isCDL: @entangle('licenses.'.$index.'.is_cdl') }">
                <div class="flex items-center mb-2">
                    <input type="checkbox" wire:model="licenses.{{ $index }}.is_cdl" id="is_cdl_{{ $index }}" x-model="isCDL" class="form-checkbox h-4 w-4 text-primary border-gray-300 rounded">
                ...
                <div x-show="isCDL" class="mt-4">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div class="flex items-center">
                            <input type="checkbox" id="endorsement_n_{{ $index }}" value="N" wire:model="licenses.{{ $index }}.endorsements" class="form-checkbox h-4 w-4 text-primary border-gray-300 rounded">
                            <label for="endorsement_n_{{ $index }}" class="ml-2 text-sm">N (Tank)</label>
                        </div>
                        <div class="flex items-center">
                            <input type="checkbox" id="endorsement_h_{{ $index }}" value="H" wire:model="licenses.{{ $index }}.endorsements" class="form-checkbox h-4 w-4 text-primary border-gray-300 rounded">
                            <label for="endorsement_h_{{ $index }}" class="ml-2 text-sm">H (HAZMAT)</label>
                        </div>
                        <div class="flex items-center">
                            <input type="checkbox" id="endorsement_x_{{ $index }}" value="X" wire:model="licenses.{{ $index }}.endorsements" class="form-checkbox h-4 w-4 text-primary border-gray-300 rounded">
                            <label for="endorsement_x_{{ $index }}" class="ml-2 text-sm">X (Combo)</label>
                        </div>
                        <div class="flex items-center">
                            <input type="checkbox" id="endorsement_t_{{ $index }}" value="T" wire:model="licenses.{{ $index }}.endorsements" class="form-checkbox h-4 w-4 text-primary border-gray-300 rounded">
                            <label for="endorsement_t_{{ $index }}" class="ml-2 text-sm">T (Double/Triple)</label>
                        </div>
                        <div class="flex items-center">
                            <input type="checkbox" id="endorsement_p_{{ $index }}" value="P" wire:model="licenses.{{ $index }}.endorsements" class="form-checkbox h-4 w-4 text-primary border-gray-300 rounded">
                            <label for="endorsement_p_{{ $index }}" class="ml-2 text-sm">P (Passenger)</label>
                        </div>
                        <div class="flex items-center">
                            <input type="checkbox" id="endorsement_s_{{ $index }}" value="S" wire:model="licenses.{{ $index }}.endorsements" class="form-checkbox h-4 w-4 text-primary border-gray-300 rounded">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                <div x-data="{
                    ...
                    <div class="flex items-center space-x-2">
                        <button type="button" @click="$refs.license_front.click()" class="px-3 py-2 bg-gray-200 rounded cursor-pointer hover:bg-gray-300 text-sm" :disabled="loading">
                        ...
                        <input type="file" x-ref="license_front" class="hidden" accept="image/*" @change="
                ...
                <div x-data="{
                    ...
                    <div class="flex items-center space-x-2">
                        <button type="button" @click="$refs.license_back.click()" class="px-3 py-2 bg-gray-200 rounded cursor-pointer hover:bg-gray-300 text-sm" :disabled="loading">
                            <span x-show="!loading">Select Image</span>
                            <span x-show="loading" class="flex items-center">
                                <svg class="animate-spin -ml-1 mr-2 h-4 w-4 text-primary" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                                    <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                        ...
                        <input type="file" x-ref="license_back" class="hidden" accept="image/*" @change="
                            const file = $event.target.files[0];
                            if(!file) return;
                            if(file.size > 2 * 1024 * 1024) {
                                error = 'File size must be less than 2MB';
        ...
        <button type="button" wire:click="addLicense" class="px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700 text-sm">
    ...
    <div class="mb-6 border-t pt-6">
        ...
        <div class="experience-entry border rounded-lg p-4 mb-4">
            <div class="flex justify-between items-center mb-4">
                ...
                @if($index > 0)
                <button type="button" wire:click="removeExperience({{ $index }})" class="px-3 py-1 bg-red-500 text-white rounded hover:bg-red-600 text-sm">
                    Remove
                </button>
                @endif
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                <div>
                    ...
                    <select wire:model="experiences.{{ $index }}.equipment_type" class="w-full text-sm border-slate-200 shadow-sm rounded-md py-2 px-3">
                ...
                <div>
                    ...
                    <input type="number" wire:model="experiences.{{ $index }}.years_experience" min="0" max="10" step="1" class="w-full text-sm border-slate-200 shadow-sm rounded-md py-2 px-3">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Total Miles Driven <span class="text-red-500">*</span></label>
                    <input type="number" wire:model="experiences.{{ $index }}.miles_driven" min="0" step="1" class="w-full text-sm border-slate-200 shadow-sm rounded-md py-2 px-3">
                ...
                <div>
                    ...
                    <div class="flex items-center">
                        <input type="checkbox" id="requires_cdl_{{ $index }}" wire:model="experiences.{{ $index }}.requires_cdl" class="form-checkbox h-4 w-4 text-primary border-gray-300 rounded">
        ...
        <button type="button" wire:click="addExperience" class="px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700 text-sm">
