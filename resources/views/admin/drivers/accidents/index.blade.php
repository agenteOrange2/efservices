        ...
        <script>
            document.addEventListener('DOMContentLoaded', function() {
                ...
                editCarrierSelect.addEventListener('change', function() {
                    const carrierId = this.value;
                    if (carrierId) {
                        // Actualizar esta línea para usar la misma ruta
                        fetch(`/api/get-drivers-by-carrier-id/${carrierId}`)
                            .then(response => response.json())
                            .then(data => {
                ...
                editButtons.forEach(button => {
                    button.addEventListener('click', function() {
                        ...
                        const carrierId = accident.user_driver_detail.carrier_id;
                        document.getElementById('edit_carrier').value = carrierId;

                        // Trigger del evento change para cargar los conductores
                        const changeEvent = new Event('change');
                        document.getElementById('edit_carrier').dispatchEvent(changeEvent);

                        // Asignar el driver una vez que se carguen las opciones (con un pequeño delay)
                        setTimeout(() => {
                            document.getElementById('edit_user_driver_detail_id').value =
                                accident.user_driver_detail_id;
                        }, 500);
                    });
                });
