...
class CoursesController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            // Procesar archivos de certificados si existen
            if ($request->filled('certificate_files')) {
                $filesData = json_decode($request->certificate_files, true);
                ...
                if (is_array($filesData)) {
                    ...
                    $destinationDir = "public/driver/{$driverId}/courses/{$course->id}";
                    if (!Storage::exists($destinationDir)) {
                        Storage::makeDirectory($destinationDir);
                    }
                    
                    foreach ($filesData as $fileData) {
                        if (!empty($fileData['name']) && isset($fileData['is_temp']) && $fileData['is_temp']) {
                            try {
                                ...
                                if (Storage::move($tempPath, $destinationPath)) {
                                    ...
                                    $document->documentable_type = DriverCourse::class;
                                    $document->documentable_id = $course->id;
                                    $document->file_path = $destinationPath;
                                    $document->file_name = $fileName;
                                    $document->original_name = $fileData['original_name'] ?? $fileName;
                                    ...
                                    Log::info('Documento de curso subido correctamente', [
                                        'course_id' => $course->id,
                                        'file_name' => $fileName
                                    ]);
                                } else {
        ...
        try {
            ...
            if ($request->filled('certificate_files')) {
                ...
                if (is_array($filesData)) {
                    ...
                    foreach ($filesData as $fileData) {
                        // Solo procesar archivos temporales (nuevos)
                        if (!empty($fileData['name']) && isset($fileData['is_temp']) && $fileData['is_temp']) {
                            try {
                                // Ruta del archivo temporal
                                $tempPath = isset($fileData['tempPath']) 
                                ...
                                if (Storage::move($tempPath, $destinationPath)) {
                                    ...
                                    Log::info('Documento de curso actualizado correctamente', [
                                        'course_id' => $course->id,
                                        'file_name' => $fileName
                                    ]);
                                } else {
        ...
        try {
            ...
            if (is_array($filesArray)) {
                foreach ($filesArray as $file) {
                    ...
                    // Usar addDocument desde el trait HasDocuments si está disponible
                    if (method_exists($course, 'addDocument')) {
                        // La colección será 'document_attachments' para seguir el estándar del sistema
                        $document = $course->addDocument(
                            $fullPath,
                        ...
                        // Usar el sistema de medios tradicional
                    ...
                    Log::info('Documento de curso subido correctamente', [
                        'course_id' => $course->id,
                        'file_name' => $file['original_name'] ?? basename($fullPath),
